...
def display_summary(search_term: str, results: dict):
    ...
    # EC Meetings (minister/representative mode)
    if results.get("ec_meetings"):
        data = results["ec_meetings"]
        with st.expander("🇪🇺 **EC Commissioner/Official Meetings** ✅", expanded=True):
            meetings = data.get("meetings", [])
            by_org = data.get("by_organisation", {})
            ...
            cols = st.columns(4)
            with cols[0]:
                st.metric("Total EC Meetings", len(meetings))
            with cols[1]:
                st.metric("Organisations Met", len(by_org))
            with cols[2]:
                st.metric("Representatives Matched", len(by_rep))
            with cols[3]:
...
def preview_matches(search_term: str, selected: dict, progress_callback=None, uk_months_back=None, search_field="organisation"):
    ...
    # EC Meetings - search by representative in minister mode
    if minister_mode:
        if progress_callback:
...
search_mode = st.sidebar.radio(
    ...
    index=0,
    help="Search by company/organisation name, or search UK & EU meetings by minister/commissioner/official name."
)
