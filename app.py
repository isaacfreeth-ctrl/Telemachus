...
def preview_matches(search_term: str, selected: dict, progress_callback=None, uk_months_back=None):
    ...
    if selected.get("uk"):
        if progress_callback:
            progress_callback("🇬🇧 Searching UK meetings...", done/total)
        matches["uk"] = search_uk_ministerial_meetings(search_term, months_back=uk_months_back)
        done += 1
...
if search_button and search_term:
    if not any(selected.values()):
        ...
        # Stage 1: Get matches
        st.session_state.matches = preview_matches(search_term, selected, update_progress, uk_months_back=uk_months)
...
if st.session_state.matches and st.session_state.search_term_used:
    ...
    # Show jurisdictions with no matches
    no_matches = []
