...
def display_summary(search_term: str, results: dict):
    ...
    # Show any warnings from data fetch
    for warning in results.get("_warnings", []):
        st.warning(warning)
    ...
    if eu_selections:
        if len(eu_selections) == 1:
            ...
            try:
                ...
                results["_warnings"].append(f"⚠️ Could not fetch EU data for {eu_selections[0]['name']}: {e}")
                results["eu"] = None
            done += 1
        else:
            ...
            for sel in eu_selections:
                ...
                try:
                    ...
                    results["_warnings"].append(f"⚠️ Could not fetch EU data for {sel['name']}: {e}")
                done += 1
    ...
    if fr_selections:
        if len(fr_selections) == 1:
            ...
            try:
                ...
                results["_warnings"].append(f"⚠️ Could not fetch France data for {fr_selections[0]['name']}: {e}")
                results["france"] = None
            done += 1
        else:
            all_entities = []
            for sel in fr_selections:
                ...
                try:
                    ...
                    results["_warnings"].append(f"⚠️ Could not fetch France data for {sel['name']}: {e}")
                done += 1
    ...
    if de_selections:
        if len(de_selections) == 1:
            ...
            try:
                ...
                results["_warnings"].append(f"⚠️ Could not fetch Germany data for {de_selections[0]['name']}: {e}")
            ...
            for sel in de_selections:
                ...
                try:
                    ...
                    results["_warnings"].append(f"⚠️ Could not fetch Germany data for {sel['name']}: {e}")
                done += 1
