...
def display_summary(search_term: str, results: dict):
    ...
    if results.get("ec_meetings"):
        ...
        expander_title = "🇪🇺 **EC Meetings on Topic** ✅" if ec_is_topic else "🇪🇺 **EC Commissioner/Official Meetings** ✅"
        with st.expander(expander_title, expanded=True):
            ...
            cols = st.columns(4)
            with cols[0]:
                st.metric("Meetings on Topic" if ec_is_topic else "Total EC Meetings", len(meetings))
            with cols[1]:
                st.metric("Organisations", len(by_org))
            with cols[2]:
            ...
            if ec_is_topic:
                # Topic mode: show who's lobbying on this topic
                if by_org:
                    st.markdown("**Top organisations meeting on this topic:**")
                    for org, count in list(by_org.items())[:15]:
                        st.write(f"• {org}: {count} meetings")
...
def preview_matches(search_term: str, selected: dict, progress_callback=None, uk_months_back=None, search_field="organisation"):
    ...
    meetings_only_mode = minister_mode or topic_mode
    ...
    if selected.get("uk"):
        if progress_callback:
            label = "🇬🇧 Searching UK meetings by minister..." if minister_mode else "🇬🇧 Searching UK meetings by topic..." if topic_mode else "🇬🇧 Searching UK meetings..."
...
search_mode = st.sidebar.radio(
    "🔍 Search mode",
    options=["Organisation", "Minister / Official", "Topic / Subject"],
    index=0,
    help="Search by company/organisation name, by minister/commissioner/official name, or by meeting topic/subject keyword."
)
...
if search_button and search_term:
    if not any(selected.values()):
        ...
        # Stage 1: Get matches
        search_field = "minister" if is_minister_search else "topic" if is_topic_search else "organisation"
...
if st.session_state.matches and st.session_state.search_term_used:
    ...
    is_meetings_only_mode = st.session_state.get("search_mode") in ("Minister / Official", "Topic / Subject")
    no_matches = []
