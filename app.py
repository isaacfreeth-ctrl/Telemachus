...
def display_summary(search_term: str, results: dict):
    ...
    # Check if this is a minister mode search
    uk_data = results.get("uk")
    ...
    if is_minister_mode:
        ...
        if uk_data and uk_data.get("meetings"):
            st.metric("UK Meetings Found", len(uk_data["meetings"]))
        else:
            st.warning("No meetings found for this minister/official name.")
    else:
...
def preview_matches(search_term: str, selected: dict, progress_callback=None, uk_months_back=None, search_field="organisation"):
    ...
    # In minister search mode, only search UK meetings
    minister_mode = search_field == "minister"
    
    # EU - just get matches, don't fetch full data
    if selected.get("eu") and not minister_mode:
        if progress_callback:
            progress_callback("🇪🇺 Searching EU register...", done/total)
    ...
    if selected.get("france") and not minister_mode:
        if progress_callback:
            progress_callback("🇫🇷 Searching France (HATVP)...", done/total)
    ...
    if selected.get("germany") and not minister_mode:
        if progress_callback:
            progress_callback("🇩🇪 Searching Germany (Bundestag)...", done/total)
    ...
    if selected.get("uk"):
        if progress_callback:
            label = "🇬🇧 Searching UK meetings by minister..." if minister_mode else "🇬🇧 Searching UK meetings..."
    ...
    if selected.get("ireland") and not minister_mode:
        if progress_callback:
            progress_callback("🇮🇪 Searching Ireland lobbying register...", done/total)
    ...
    if selected.get("netherlands") and not minister_mode:
        if progress_callback:
            progress_callback("🇳🇱 Searching Netherlands agendas...", done/total)
    ...
    if selected.get("austria") and not minister_mode:
        if progress_callback:
            progress_callback("🇦🇹 Searching Austria...", done/total)
    ...
    if selected.get("catalonia") and not minister_mode:
        if progress_callback:
            progress_callback("🏴󠁥󠁳󠁣󠁴󠁿 Searching Catalonia...", done/total)
    ...
    if selected.get("finland") and not minister_mode:
        if progress_callback:
            progress_callback("🇫🇮 Searching Finland...", done/total)
    ...
    if selected.get("slovenia") and not minister_mode:
        if progress_callback:
            progress_callback("🇸🇮 Searching Slovenia...", done/total)
...
search_mode = st.sidebar.radio(
    "🔍 Search mode",
    options=["Organisation", "Minister / Official"],
    index=0,
    help="Search by company/organisation name, or search UK meetings by minister/official name."
)
is_minister_search = search_mode == "Minister / Official"

if is_minister_search:
    st.sidebar.caption("🇬🇧 Minister search uses the UK meetings index only. Other jurisdictions are disabled.")
...
with col1:
    if is_minister_search:
        search_term = st.text_input(
            "🔍 Minister or official name",
            placeholder="e.g. Gareth Davies, Rachel Reeves...",
            help="Search UK ministerial & senior official meetings by name"
        )
    else:
        search_term = st.text_input(
            "🔍 Company name", 
...
if search_button and search_term:
    if not any(selected.values()):
        ...
        # Stage 1: Get matches
        search_field = "minister" if is_minister_search else "organisation"
...
if st.session_state.matches and st.session_state.search_term_used:
    ...
    # Show jurisdictions with no matches (skip in minister mode since only UK is searched)
    is_minister_search_mode = st.session_state.get("search_mode") == "Minister / Official"
    no_matches = []
    if not is_minister_search_mode:
