...
def get_csv_urls_from_publication(pub_path):
    ...
    Returns list of tuples: (department, csv_url, publication_link)
    """
    ...
    for doc in data.get("details", {}).get("documents", []):
        if isinstance(doc, str) and ".csv" in doc.lower():
            ...
            for u in urls:
                ...
                csv_urls.append((dept, u, pub_path))
    ...
    for attachment in data.get("details", {}).get("attachments", []):
        ...
        if ".csv" in url.lower():
            ...
            csv_urls.append((dept, url, pub_path))
    
    return csv_urls


def download_and_parse_csv(url, pub_link=None):
    """Download a CSV and parse all meetings from it.
    
    Args:
        url: The CSV download URL
        pub_link: The GOV.UK publication page URL (for citation)
    """
    
    meetings = []
    
    # Create citation-friendly source URL (the publication page, not the raw CSV)
    if pub_link:
        source_url = f"https://www.gov.uk{pub_link}" if pub_link.startswith("/") else pub_link
    else:
        # Fall back to CSV URL if no publication link
        source_url = url
...
def build_index():
    ...
    for i, pub in enumerate(ministerial_pubs):
        ...
        for dept, url, pub_link in csv_urls:
            all_csvs.append({"dept": dept, "url": url, "type": "ministerial", "pub_link": pub_link})
    ...
    for i, pub in enumerate(senior_pubs):
        ...
        csv_urls = get_csv_urls_from_publication(pub["link"])
        for dept, url, pub_link in csv_urls:
            all_csvs.append({"dept": dept, "url": url, "type": "senior_officials", "pub_link": pub_link})
    ...
    for i, csv_info in enumerate(unique_csvs):
        ...
        meetings = download_and_parse_csv(csv_info["url"], csv_info.get("pub_link"))
